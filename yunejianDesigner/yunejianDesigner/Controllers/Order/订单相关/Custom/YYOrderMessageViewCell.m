...
-(void)updateUI{
    ...
    if(_msgInfoModel.msgContent){
        _buyerLabel.text =  [NSString stringWithFormat:@"%@%@",_msgInfoModel.msgContent.buyerName ,([_msgInfoModel.isAppendOrder integerValue]?NSLocalizedString(@"（追单）",nil):@"")];
    }else{
        _buyerLabel.text = @"";
    }
    if(_msgInfoModel.msgContent && ![NSString isNilOrEmpty:_msgInfoModel.msgContent.op] && [_msgInfoModel.msgContent.op isEqualToString:@"pack_ex"]){
        _orderCodeLabel.text =  [NSString stringWithFormat:@"%@%@",NSLocalizedString(@"物流公司：",nil),_msgInfoModel.msgContent.logisticsName];//
        _orderTimerLabel.text = [NSString stringWithFormat:@"%@%@",NSLocalizedString(@"物流单号：",nil),_msgInfoModel.msgContent.LogisticsCode];
