    ...
    if (indexPath.section == 0) {
        if(indexPath.row == 0){
            ...
            if(!buyerMessageCell){
                ...
                //选择订单类型这块
                [buyerMessageCell setOrderTypeButtonClicked:^{

                    [ws showPickView:@[@"买断（Buy out）",@"寄售（Consignment sale）"] type:UIDataTypeOrderType];
            ...
            [taxInfoCell setTaxChooseBlock:^{
                ...
                [chooseTaxView setSelectBlock:^(NSInteger selectIndex){
                    ...
                    [ws.navigationController popViewControllerAnimated:YES];
                    [self btnClick:indexPath.row section:indexPath.section andParmas:@[@"taxType",@(ws.selectTaxType)]];
                    NSLog(@"111");
                }];
        ...
        [orderRemarkCell setBuyerButtonClicked:^(UIView *view){
            ...
            if(user.userType != YYUserTypeShowroom && user.userType != YYUserTypeShowroomSub && user.userType != YYUserTypeSales)
            {
                ws.salesManList = [[NSMutableArray alloc] init];
                ...
                [YYUserApi getSalesManListWithBlockNew:^(YYRspStatusAndMessage *rspStatusAndMessage, YYSalesManListModel *salesManListModel, NSError *error) {
                    if (rspStatusAndMessage.status == YYReqStatusCode100) {
                        ...
                        for (YYSalesManModel* model in ws.salesManListModel.result) {
                            if([model.userType integerValue] == YYUserTypeShowroom || [model.userType integerValue] == YYUserTypeShowroomSub)
                            {
    ...
    if (indexPath.section == 0) {
        if(indexPath.row == 0){
            if( _nowBuyerAddress == nil ){
                return 390 - 106 +48 + 60;
            }
            return 390 - 106 + 90 + 60;
        }else if(indexPath.row == 1){
...
-(void)toobarDonBtnHaveClick:(YYPickView *)pickView resultString:(NSString *)resultString{
    ...
    if (pickView.uidataType == UIDataTypeSalesManList && self.salesManList) {
        ...
        [self.tableView reloadData];
    }else if (pickView.uidataType == UIDataTypeOrderType){
